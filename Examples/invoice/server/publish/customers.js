Meteor.publish("customer_list", function() {
	return Customers.find({ownerId:this.userId}, {transform:function(doc) {var sum = 0;Invoices.find({ customerId: doc._id }).map(function(item) {sum += item.totalAmount;});doc.totalAmount = sum;return doc;}});
});
